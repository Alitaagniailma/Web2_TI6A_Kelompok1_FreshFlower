...
(1, 'Administrator', 'admin', 'admin123', 'admin@freshflower.com');
...
,`tanggal` datetime
,`total` decimal(12,2)
,`status` enum('Menunggu Pembayaran','Diproses','Dikirim','Selesai','Batal')
);
...
INSERT INTO `pelanggan` (`id_pelanggan`, `nama`, `email`, `password`, `alamat`, `no_hp`) VALUES
(1, 'Yuyun Yunengsih', 'yuyun@gmail.com', '123456', 'Bandung', '081234567890');
...
CREATE TABLE `produk` (
  ...
  `id_kategori` int(11) NOT NULL,
  `nama_bunga` varchar(150) NOT NULL,
...
INSERT INTO `produk` (`id_produk`, `id_kategori`, `nama_bunga`, `harga`, `stok`, `deskripsi`, `foto`) VALUES
(1, 1, 'Mawar Merah', 75000.00, 25, 'Bunga mawar merah segar', 'mawar.jpg'),
(2, 2, 'Tulip Putih', 95000.00, 20, 'Tulip import premium', 'tulip.jpg'),
(3, 3, 'Lily Putih', 85000.00, 15, 'Lily putih elegan', 'lily.jpg'),
(4, 4, 'Anggrek Bulan', 125000.00, 12, 'Anggrek bulan pilihan', 'anggrek.jpg'),
(5, 5, 'Buket Wisuda', 180000.00, 10, 'Buket bunga wisuda', 'wisuda.jpg');
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `laporan_penjualan`  AS SELECT `p`.`id_pesanan` AS `id_pesanan`, `pl`.`nama` AS `nama`, `p`.`tanggal` AS `tanggal`, `p`.`total` AS `total`, `p`.`status` AS `status` FROM (`pesanan` `p` join `pelanggan` `pl` on(`p`.`id_pelanggan` = `pl`.`id_pelanggan`)) ;
